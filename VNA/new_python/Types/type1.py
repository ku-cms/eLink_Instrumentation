...
with open("Data/" + config.cable_number + "/Impedence_List.csv", "a") as Ana2:
    Ana2.write(
        "Cable Number,Length,Type, Parameter, CMD (Normal),CMD (2-5), D0 (Normal), D0 (2-5), D1(Normal), D1 (2-5), D2(Normal), D2 (2-5), D3(Normal), D3 (2-5),\n")
    Ana2.close()
...
while Breaker == True:
    ...
    with style.context('seaborn-darkgrid'):
        ...
        net8_dc.plot_s_db(label='S' + config.comp + ff4.split('.vna')[0].split('/')[-1:][0], ax=ax0, color='w')
        net10_dc.plot_s_db(label='S' + config.comp + ff5.split('.vna')[0].split('/')[-1:][0], ax=ax0, color='m')
        ...
        net8_dc.plot_z_time_step(pad=0, window='hamming', z0=50,
                                 label='TD' + config.comp + ff4.split('.vna')[0].split('/')[-1:][0], ax=ax1, color='w')
        config.display_mean_impedance(ax1, config.t1, config.t2, config.dt1, config.dt2, 'w')

        net10_dc.plot_z_time_step(pad=0, window='hamming', z0=50,
                                  label='TD' + config.comp + ff5.split('.vna')[0].split('/')[-1:][0], ax=ax1, color='m')
        config.display_mean_impedance(ax1, config.t1, config.t2, config.dt1, config.dt2, 'm')

        with open("Records/Impedence_List_type1.csv", "a") as Ana:
            Ana.write(
