...
with open("Data/" + config.cable_number + "/Impedence_List.csv", "a") as Ana2:
    Ana2.write(
        "Cable Number,Length,Type, Parameter, M1CMD (Normal),M1CMD (2-5), M1D0 (Normal), M1D0 (2-5), M1D1(Normal), M1D1 (2-5), M2CMD (Normal),M2CMD (2-5), M2D0 (Normal), M2D0 (2-5), M2D1(Normal), M2D1 (2-5),M3CMD (Normal),M3CMD (2-5), M3D0 (Normal), M3D0 (2-5), M3D1(Normal), M3D1 (2-5),\n")
    Ana2.close()
...
while Breaker == True:
    ...
    net12 = rf.Network(
        ...
        f_unit='ghz')
    net14 = rf.Network(
        "Data/" + config.cable_number + "/Plots/s2p/" + filename7.replace(".txt", "") + '_' + config.subfile + '.s2p',
        f_unit='ghz')
    net16 = rf.Network(
        "Data/" + config.cable_number + "/Plots/s2p/" + filename8.replace(".txt", "") + '_' + config.subfile + '.s2p',
        f_unit='ghz')
    net18 = rf.Network(
        "Data/" + config.cable_number + "/Plots/s2p/" + filename9.replace(".txt", "") + '_' + config.subfile + '.s2p',
        f_unit='ghz')

    with style.context('seaborn-darkgrid'):

        fig0 = plt.figure(figsize=(40, 20))
        ...
        ax4.grid(True, color='0.8', which='minor')
        ax4.grid(True, color='0.4', which='major')
        ...
        net14_dc = net14[i, j].extrapolate_to_dc(kind='linear')
        net16_dc = net16[i, j].extrapolate_to_dc(kind='linear')
        net18_dc = net18[i, j].extrapolate_to_dc(kind='linear')
        ...
        net14_dc.plot_s_db(label='S' + config.comp + ff7.split('.vna')[0].split('/')[-1:][0], ax=ax4, color='b')
        net16_dc.plot_s_db(label='S' + config.comp + ff8.split('.vna')[0].split('/')[-1:][0], ax=ax4, color='r')
        net18_dc.plot_s_db(label='S' + config.comp + ff9.split('.vna')[0].split('/')[-1:][0], ax=ax4, color='g')
        ...
        net14_dc.plot_z_time_step(pad=0, window='hamming', z0=50,
                                  label='TD' + config.comp + ff7.split('.vna')[0].split('/')[-1:][0], ax=ax5, color='b')
        config.display_mean_impedance(ax5, config.t1, config.t2, config.dt1, config.dt2, 'b')

        net16_dc.plot_z_time_step(pad=0, window='hamming', z0=50,
                                  label='TD' + config.comp + ff8.split('.vna')[0].split('/')[-1:][0], ax=ax5, color='r')
        config.display_mean_impedance(ax5, config.t1, config.t2, config.dt1, config.dt2, 'r')

        net18_dc.plot_z_time_step(pad=0, window='hamming', z0=50,
                                  label='TD' + config.comp + ff9.split('.vna')[0].split('/')[-1:][0], ax=ax5, color='g')
        config.display_mean_impedance(ax5, config.t1, config.t2, config.dt1, config.dt2, 'g')

        with open("Records/Impedence_List_type4.csv", "a") as Ana:
            Ana.write(
